            ...
            itemBuilder: (context, index) {
              ...
              return Padding(
                padding: EdgeInsets.only(
                  left: index == 0 ? 16 : 8,
                  right: index == widget.activities.length - 1 ? 16 : 8,
                ),
                child: _ActivityItem(
                  activity: activity,
                  onVideoTap: activity.videoUrl != null
                      ? () => _launchVideo(activity.videoUrl!)
                      : null,
                ),
              );
            },
...
class _ActivityItem extends StatelessWidget {
  ...
  const _ActivityItem({required this.activity, this.onVideoTap});
  ...
  @override
  Widget build(BuildContext context) {
    final isVideo = activity.videoUrl != null;
    ...
    if (isVideo) {
      ...
      if (activity.videoCoverUrl != null &&
          activity.videoCoverUrl!.isNotEmpty) {
        // Cloudinary URLs are already absolute, no need to resolve
        imageUrl =
            activity.videoCoverUrl!.startsWith('http://') ||
                activity.videoCoverUrl!.startsWith('https://')
            ? activity.videoCoverUrl!
            : resolveFileUrl(activity.videoCoverUrl!);
      } else {
        imageUrl = _getVideoThumbnail(activity.videoUrl!);
      }
    } else {
      imageUrl = activity.imageUrl != null
          ? resolveFileUrl(activity.imageUrl!)
          : null;
    }
